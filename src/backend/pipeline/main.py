...
def parse_arguments():
    ...
    parser.add_argument('--min-fold', type=float, default=2,
...
def main(wd, min_fold, maxquant_analysis_is_needed, enzymes):
    ...
    edit_progress(html_path, progress=10)
    ...
    raw_output_dir = f'{output_dir}/text'
    ...
    if maxquant_analysis_is_needed and \
        ...
        time_passed = wait_for_maxquant(el_raw_path)
        time_passed = wait_for_maxquant(ft_raw_path, time_passed)
    ...
    edit_progress(html_path, progress=60)

    # filter maxquant resulted peptides
    logger.info('Filtering MaxQuant results...')
    filtered_peptides_info = filter_peptides(elution_peptides_path, flowthrough_peptides_path, raw_output_dir, min_fold)
    edit_progress(html_path, progress=70)
    ...
    non_informative_path = f'{raw_output_dir}/non_informative.csv'
    informative_path = f'{raw_output_dir}/informative.csv'
    cdr3_informative_path = f'{raw_output_dir}/cdr3_informative.csv'
    if not os.path.exists(cdr3_informative_path):
        logger.info('No classifications were detected. Classifying filtered results (non-info/info/cdr3 info)...')
        classify_peptides(db_paths, filtered_peptides_info, non_informative_path, informative_path, cdr3_informative_path)
    else:
        logger.info('_'*100)
    ...
    if not os.path.exists(plots_output_dir):
        ...
        plot_results(cdr3_informative_path, plots_output_dir)
    else:
    ...
    edit_success_html(html_path, run_number, output_dir)
...
if __name__ == '__main__':
    try:
        ...
        edit_failure_html(html_path, run_number, e.strerror)
